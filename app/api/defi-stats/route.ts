...
export async function GET() {
  try {
    ...
    // Attempt 2: DefiLlama BTC price chart (free, no rate limit)
    if (btcPriceHistory.length === 0) {
      const btcLlamaRes = await fetch(
        'https://api.llama.fi/v2/historicalChainTvl/Bitcoin',
        { headers: { 'Accept': 'application/json' }, next: { revalidate: 300 } }
      ).catch(() => null)

      if (btcLlamaRes?.ok) {
        const btcLlamaData = await btcLlamaRes.json()
        // llama returns { date, tvl } — tvl of Bitcoin chain ≈ BTC price × locked BTC
        // Use CryptoCompare as final fallback instead
      }

      // Attempt 3: CryptoCompare (free, high rate limits)
